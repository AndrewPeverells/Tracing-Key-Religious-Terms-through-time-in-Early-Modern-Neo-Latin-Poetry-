## Tracing Key Religious Terms Through Time in Early Modern Neo-Latin Poetry

This repository contains the datasets, metadata, and pipeline used for the project “Tracing Key Religious Terms Through Time in Early Modern Neo-Latin Poetry.”
The goal of this study is to examine how the semantics and contextual associations of major theological concepts — most centrally gratia and fides — changed across time, place, and confessional background in early modern Latin poetry of the German lands (15th–19th c.).

The repository includes:

- A curated and cleaned corpus derived from the CAMENA – Corpus Automatum Multiplex Electorum Neolatinitatis Auctorum (University of Mannheim), redistributed under the terms of the CC BY-SA 4.0 license;

- Enriched metadata for authors, printers, places, and publication dates;

- Scripts for text extraction, preprocessing, metadata alignment, and collocate statistics for implementing semantic drift analysis and diachronic collocate evolution;

- Visualisations of term trajectories across time periods and confessional contexts.



## Data Licensing & Attribution

This repository redistributes and transforms texts from the CAMENA – Corpus Automatum Neolatinum (University of Mannheim).

Original source:
https://github.com/nevenjovanovic/camena-neolatinlit


https://mateo.uni-mannheim.de/camenahtdocs/camenapoem_e.html
License:
All texts in /data/ are licensed under the Creative Commons Attribution-ShareAlike 4.0 International License (CC BY-SA 4.0).
https://creativecommons.org/licenses/by-sa/4.0/

Modifications by Andrea Peverelli and Barbara McGillivray:

cleaning of XML

removal of paratext

extraction of poetic text

normalisation of spelling

metadata reconstruction and enrichment

######

If you use this repository for research, please cite both:

- The CAMENA Project, as the original source of the texts

- This repository, for the cleaned corpus, metadata, and computational workflows




## Authors
- Dr. Andrea Peverelli
- Dr. Barbara McGillivray
